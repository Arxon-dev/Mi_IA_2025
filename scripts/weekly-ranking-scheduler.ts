...
// Reemplazar fetchWeeklyRankingData por una versión robusta que no depende de la relación responses
async function fetchWeeklyRankingData(): Promise<WeeklyRankingUser[]> {
  console.log('📊 Obteniendo datos del ranking semanal FIJO (robusto)...');
  try {
    const { weekStart, weekEnd, weekLabel } = getFixedWeekDates();
    console.log(`🗓️ Semana fija (${weekLabel}): ${weekStart.toISOString()} hasta: ${weekEnd.toISOString()}`);
    // 1. Agrupar respuestas por usuario
    const weeklyStats = await prisma.telegramresponse.groupBy({
      ...
      _sum: { points: true, responsetime: true },
      _count: { id: true },
    });
    // 2. Buscar datos de usuario y calcular estadísticas
    const users = await Promise.all(
      weeklyStats.map(async stat => {
        ...
        const weeklyResponses = stat._count.id;
        const weeklyIncorrect = weeklyResponses - weeklyCorrect;
        const weeklyAccuracy = weeklyResponses > 0 ? Math.round((weeklyCorrect / weeklyResponses) * 100) : 0;
        const avgResponseTime = weeklyResponses > 0 && stat._sum.responsetime ? Math.round(stat._sum.responsetime / weeklyResponses) : 0;
        return {
    ...
    const weeklyUsers = users.filter(Boolean) as WeeklyRankingUser[];
