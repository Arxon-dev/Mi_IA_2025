...
// Muestra qué torneos se crearán sin crearlos realmente
...
function generateTournamentDates(startDate: Date, enddate: Date): Date[] {
  ...
  while (current <= endDate) {
    ...
    // Si es uno de los días de torneo
...
function main() {
  console.log('🏆 VISTA PREVIA - TORNEOS PERMA PROGRAMADOS');
  console.log('===========================================');
  console.log(`📅 Período: ${START_DATE.toLocaleDateString('es-ES')} - ${END_DATE.toLocaleDateString('es-ES')}`);
  console.log(`📋 Configuración:`);
  console.log(`   • Días: ${TOURNAMENT_DAYS.map(d => getDayName(d)).join(', ')}`);
  console.log(`   • Horarios: ${TOURNAMENT_TIMES.join(', ')}`);
  console.log(`   • Preguntas: 20 por torneo`);
  ...
  for (const [month, dates] of byMonth) {
    console.log(`\n📅 ${month}: ${dates.length} torneos`);
    
    // Agrupar por día de la semana en este mes
  ...
  if (tournamentDates.length > 20) {
    console.log(`    ... y ${tournamentDates.length - 20} torneos más`);
  }

  // Mostrar los últimos 5 torneos
  ...
  console.log('\n📈 ESTADÍSTICAS:');
  console.log('================');
  console.log(`🎯 Total de torneos: ${tournamentDates.length}`);
  console.log(`⏰ Duración total estimada: ${(tournamentDates.length * 20 / 60).toFixed(1)} horas`);
  console.log(`🎮 Preguntas totales: ${tournamentDates.length * 20} preguntas`);
  console.log(`📅 Período: ${Math.ceil((END_DATE.getTime() - START_DATE.getTime()) / (1000 * 60 * 60 * 24))} días`);
  console.log(`📊 Promedio: ${(tournamentDates.length / Math.ceil((END_DATE.getTime() - START_DATE.getTime()) / (1000 * 60 * 60 * 24 * 7))).toFixed(1)} torneos por semana`);

  console.log('\n✅ Para ejecutar la creación real, ejecuta:');
