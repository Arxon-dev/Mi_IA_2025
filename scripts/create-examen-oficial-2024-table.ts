...
async function createExamenOficial2024Table() {
  try {
    console.log('🏗️ CREANDO TABLA ExamenOficial2024');
    console.log('==================================');

    // El esquema de la tabla se debe agregar al schema.prisma primero
    // Esta función verificará si la tabla existe y puede usarse

    // Verificar si la tabla existe intentando hacer una consulta simple
    try {
      const count = await prisma.examenOficial2024.count();
      console.log(`✅ Tabla ExamenOficial2024 ya existe con ${count} registros`);
    } catch (error) {
      console.error('❌ La tabla ExamenOficial2024 no existe.');
      console.log('\n📋 NECESITAS AGREGAR AL SCHEMA.PRISMA:');
      console.log(`
model ExamenOficial2024 {
  ...
  @@map("ExamenOficial2024")
      ...
      `);
      console.log('\n🔧 DESPUÉS EJECUTA: npx prisma db push');
      return;
    }
