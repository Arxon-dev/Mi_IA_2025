...
async function findCapitalQuestion() {
  try {
    console.log('🔍 BUSCANDO PREGUNTA DE LA CAPITAL EN ExamenOficial2018...\n');
    ...
    if (madridQuestion) {
      ...
      console.log(`   Categoría: ${madridQuestion.category || 'N/A'}`);
      console.log(`   Activa: ${madridQuestion.isactive}`);
      console.log(`   Enviada ${madridQuestion.sendCount} veces`);
    } else {
      console.log('✅ NO SE ENCONTRÓ pregunta de capital');
    }
    ...
    if (basicGeoQuestions.length > 0) {
      console.log(`\n⚠️ PREGUNTAS DE GEOGRAFÍA BÁSICA ENCONTRADAS (${basicGeoQuestions.length}):`);
