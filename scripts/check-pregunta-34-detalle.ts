...
async function checkPregunta34Detalle() {
  try {
    console.log('🔍 ANÁLISIS DETALLADO DE LA PREGUNTA 34');
    console.log('=====================================');

    // 1. Buscar la pregunta 34 específica de ExamenOficial2018
    const pregunta34 = await prisma.examenOficial2018.findFirst({
      where: { questionnumber: 34 }
    });

    if (!pregunta34) {
      console.error('❌ Pregunta 34 no encontrada en ExamenOficial2018');
      return;
    }

    console.log('✅ Pregunta 34 encontrada:');
    console.log(`   ID: ${pregunta34.id}`);
    console.log(`   Número: ${pregunta34.questionnumber}`);
    console.log(`   Categoría: ${pregunta34.category}`);
    ...
    console.log(`   Índice respuesta correcta: ${pregunta34.correctanswerindex}`);
    ...
    // 2. Verificar si alguna opción excede el límite de Telegram (100 chars)
    console.log(`\n🚨 VERIFICACIÓN DE LÍMITES DE TELEGRAM:`);
    const longOptions = pregunta34.options.filter(opt => opt.length > 100);
    if (longOptions.length > 0) {
      console.log(`   ❌ ${longOptions.length} opciones exceden 100 caracteres:`);
      longOptions.forEach((opt, idx) => {
        console.log(`      "${opt}" (${opt.length} chars)`);
      });
    } else {
      console.log(`   ✅ Todas las opciones son válidas (<= 100 chars)`);
    }

    // 3. Verificar el total del poll
    const header = `🎯 <b>SIMULACRO</b> ⏰ 34/100\n⏱️ 2h 30m\n\n`;
    ...
    console.log(`   ¿Excede 300 chars?: ${totalLength > 300 ? '❌ SÍ' : '✅ NO'}`);
    ...
    // Validaciones que podría hacer Telegram
    if (pollData.question.length > 300) {
      console.log(`   ❌ FALLO: Pregunta excede 300 caracteres`);
    } else if (pollData.options.length < 2) {
      console.log(`   ❌ FALLO: Menos de 2 opciones`);
    } else if (pollData.options.length > 10) {
      console.log(`   ❌ FALLO: Más de 10 opciones`);
    } else if (pollData.options.some(opt => opt.length > 100)) {
      console.log(`   ❌ FALLO: Alguna opción excede 100 caracteres`);
    } else if (pollData.correctanswerindex >= pollData.options.length) {
      console.log(`   ❌ FALLO: Índice de respuesta correcta inválido`);
    } else {
