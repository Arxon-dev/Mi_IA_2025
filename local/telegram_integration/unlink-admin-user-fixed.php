...
echo "<h2>Script para Eliminar Vinculación del Usuario Administrador (Versión Corregida)</h2>";
...
function safe_get_records($table, $conditions) {
    global $DB;
    try {
        if (!table_exists($table)) {
            return false;
        }
        return $DB->get_records($table, $conditions);
    } catch (Exception $e) {
...
function safe_delete_records($table, $conditions) {
    global $DB;
    try {
        if (!table_exists($table)) {
            return false;
...
// Lista de posibles nombres de tablas para vinculación
$linkage_tables = [
...
// Lista de posibles nombres de tablas para verificación
$verification_tables = [
...
// Lista de posibles nombres de tablas para rendimiento
$performance_tables = [
...
// Lista de posibles nombres de tablas para timeline
$timeline_tables = [
...
if (isset($existing_tables['linkage'])) {
    $current_link = safe_get_records($existing_tables['linkage'], 
        array('moodle_user_id' => $moodle_user_id));
    
    if ($current_link && !empty($current_link)) {
        $link = reset($current_link); // Obtener el primer registro
...
if (isset($existing_tables['verification'])) {
    $verification_codes = safe_get_records($existing_tables['verification'], 
        array('moodle_user_id' => $moodle_user_id));
...
if (isset($existing_tables['performance'])) {
    $performance_records = safe_get_records($existing_tables['performance'], 
        array('moodle_user_id' => $moodle_user_id));
...
if (isset($existing_tables['timeline'])) {
    $timeline_records = safe_get_records($existing_tables['timeline'], 
        array('moodle_user_id' => $moodle_user_id));
...
if (isset($_POST['confirm_delete'])) {
    ...
    try {
        ...
        if (isset($existing_tables['linkage']) && $current_link && !empty($current_link)) {
            if (safe_delete_records($existing_tables['linkage'], 
                array('moodle_user_id' => $moodle_user_id))) {
        ...
        if (isset($existing_tables['verification']) && $verification_codes && !empty($verification_codes)) {
            if (safe_delete_records($existing_tables['verification'], 
                array('moodle_user_id' => $moodle_user_id))) {
        ...
        if (isset($existing_tables['performance']) && $performance_records && !empty($performance_records)) {
            if (safe_delete_records($existing_tables['performance'], 
                array('moodle_user_id' => $moodle_user_id))) {
        ...
        if (isset($existing_tables['timeline']) && $timeline_records && !empty($timeline_records)) {
            if (safe_delete_records($existing_tables['timeline'], 
                array('moodle_user_id' => $moodle_user_id))) {
