...
/**
 * Interfaz para representar una sección de documento
 */
...
/**
 * Información sobre el progreso de procesamiento de un documento
 */
...
export enum ProcessingMode {
  ...
  PARAGRAPHS = 'paragraphs',    // Por párrafos
  CUSTOM = 'CUSTOM',
  GENERIC = 'GENERIC',
  ...
  PDC_01_DOCTRINE = 'PDC_01_DOCTRINE' // Modo específico para PDC-01 B Doctrina para el empleo de las FAS
}

/**
 * Configuración para el procesamiento de secciones
 */
export interface ProcessingConfig {
  ...
  options: {
    ...
    numbered?: {
      pattern?: string;      // Patrón de números personalizado
      includeSubsections?: boolean;
    };
    hierarchical?: {
      levels?: string[];     // Nombres de los niveles jerárquicos
      maxDepth?: number;     // Profundidad máxima de la jerarquía
    };
    paragraphs?: {
    ...
    militaryDoctrine?: {
      detectChapters?: boolean;        // Detectar CAPÍTULO X
      detectMainSections?: boolean;    // Detectar secciones numeradas (1., 2., 3.)
      detectSubSections?: boolean;     // Detectar subsecciones (3.1., 3.2.)
      groupParagraphs?: boolean;       // Agrupar párrafos numerados
      paragraphsPerGroup?: number;     // Párrafos por grupo
    };
    pdc01Doctrine?: {
      useSpecificSections?: boolean;   // Usar apartados específicos predefinidos
      includeSubParagraphs?: boolean;  // Incluir párrafos numerados dentro de cada apartado
      minSectionLength?: number;       // Longitud mínima por apartado
    };
  };
}
...
export class DocumentSectionService {
  ...
  // Configuración específica para documentos militares
  ...
  // Configuración específica para PDC-01 B Doctrina para el empleo de las FAS
  ...
  static async updateDocument(document: StoredDocument): Promise<StoredDocument> {
    if (!document || !document.id) {
      throw new Error('Documento inválido: falta ID');
    }

    if (!document.content || document.content.trim() === '') {
      throw new Error('Documento inválido: contenido vacío');
    }
    ...
    try {
      ...
      if (!document.type) {
        document.type = 'text/plain'; // Valor por defecto si no está definido
      ...
      return await this.measurePerformance('procesamiento_documento', async () => {
        // 🔍 DEBUGGING: Detectar tipo de documento
        ...
        console.log('🔍 [DEBUG] Detección automática:', {
        ...
        // ✅ APLICAR DETECCIÓN AUTOMÁTICA SOLO SI NO HAY CONFIGURACIÓN PERSONALIZADA GUARDADA
        // Verificar si existe una configuración específica guardada por el usuario
        ...
        if (!hasUserConfig && (isPDC01 || isMilitary)) {
          this.log(LogLevel.INFO, 'Aplicando configuración automática durante la carga del documento (configuración por defecto detectada)');
          console.log('🔍 [DEBUG] Aplicando configuración automática:', { isPDC01, isMilitary, hasUserConfig });
          ...
          // Recargar la configuración después de aplicar la detección automática
          const updatedConfig = this.getProcessingConfig(document.id);
          console.log('🔍 [DEBUG] Configuración actualizada:', { mode: updatedConfig.mode });
        } else {
          console.log('🔍 [DEBUG] Respetando configuración del usuario:', { 
        ...
        this.log(LogLevel.INFO, 'Secciones extraídas', { count: sections.length });
        
        // 🔍 DEBUGGING: Resultado de extracción
        console.log('🔍 [DEBUG] Secciones extraídas:', {
        ...
        try {
          const response = await fetch('/api/sections', {
            ...
            body: JSON.stringify({ data: sections, documentId: document.id }),
          });
        ...
        // (Opcional) Guardar en localStorage como caché
  ...
  static async getSections(documentId: string): Promise<DocumentSection[]> {
    try {
      ...
      // Extraer las secciones usando la configuración actual
      const sections = this.extractSections(document);
      
      this.log(LogLevel.INFO, `Extraídas ${sections.length} secciones`, { 
        mode: config.mode,
        documentId 
      });
  ...
  /**
   * Guarda las preguntas generadas para una sección
   */
  static saveSectionQuestions(sectionId: string, questions: string): void {
    try {
      ...
      console.error('Error al guardar las preguntas de la sección:', error);
    }
  }

  /**
   * Marca una sección como procesada y guarda sus preguntas
   */
  ...
  /**
   * Obtiene el progreso de generación de preguntas para un documento
   */
  ...
  /**
   * Guarda el progreso de generación de preguntas para un documento
   */
  ...
  /**
   * Obtiene las preguntas generadas para una sección
   */
  ...
  /**
   * Guarda una sección de documento
   */
  ...
  /**
   * Guarda múltiples secciones de documento
   */
  ...
  /**
   * Obtiene secciones para un documento específico
  ...
  /**
   * Extrae secciones de un documento basado en su estructura y configuración
   * @param document Documento a procesar
   * @returns Array de secciones extraídas
   */
  static extractSections(document: StoredDocument): DocumentSection[] {
    try {
      const config = this.getProcessingConfig(document.id);
      this.log(LogLevel.INFO, 'Iniciando extracción de secciones', { 
        mode: config.mode,
        documentId: document.id 
      });
      
      // 🔍 DEBUGGING: Información detallada del procesamiento
      console.log('🔍 [DEBUG] extractSections - Configuración:', {
        documentId: document.id,
        title: document.title,
        mode: config.mode,
      ...
      switch (config.mode) {
        ...
        case ProcessingMode.FULL:
          // Devuelve el documento completo como una sola sección
          return [{
  ...
  private static extractNumberedSections(document: StoredDocument, config: ProcessingConfig): DocumentSection[] {
    try {
      ...
      // Si no hay secciones principales, usar el método original
      if (mainSections.length === 0) {
        ...
        for (let i = 0; i < sectionStarts.length; i++) {
          ...
          try {
            ...
            this.log(LogLevel.ERROR, 'Error al procesar sección individual', { 
              sectionIndex: i, 
              error: sectionError 
            });
            continue;
        ...
        // Procesar secciones principales y sus subsecciones como una única sección
        for (let i = 0; i < mainSections.length; i++) {
          ...
          try {
            ...
            this.log(LogLevel.ERROR, 'Error al procesar sección principal', { 
              sectionIndex: i, 
              error: sectionError 
            });
            continue;
      ...
      if (sections.length === 0) {
        this.log(LogLevel.WARN, 'No se encontraron secciones numeradas, creando sección genérica');
  ...
  /**
   * Extrae secciones jerárquicas (Capítulos, Disposiciones, etc.)
   */
  private static extractHierarchicalSections(document: StoredDocument, config: ProcessingConfig): DocumentSection[] {
    ...
    try {
      ...
      this.log(LogLevel.INFO, 'Procesando niveles jerárquicos', { levels });
      
      // Debug: Buscar una muestra del contenido para verificar formato
      const contentSample = content.substring(0, 2000);
      const articleMatches = contentSample.match(/Artículo\s*\d+/gi);
      this.log(LogLevel.DEBUG, 'Muestra de artículos en el contenido', { 
        muestra: contentSample.substring(0, 500),
        articulosEnMuestra: articleMatches 
      });

      // Crear patrón para detectar niveles (más flexible para artículos)
      // Detecta: Artículo 66, ARTÍCULO 67, etc.
      const articlePattern = /(?:Artículo|ART[IÍ]CULO)\s*\d+[.:]?/gmi;
      // Crear patrón para otros niveles
      const otherLevels = levels.filter(lvl => !['Artículo', 'ARTÍCULO', 'Art.', 'ART.'].includes(lvl));
      const otherPattern = otherLevels.length > 0
        ? new RegExp(`(?:${otherLevels.join('|')})\\s*\\d+[.:]?`, 'gmi')
        : null;

      // Encontrar todos los índices de inicio de sección
      ...
      this.log(LogLevel.INFO, `Análisis jerárquico completado`, {
      ...
      for (let i = 0; i < sectionStarts.length; i++) {
        try {
          ...
          this.log(LogLevel.ERROR, 'Error al procesar sección jerárquica individual', { 
            sectionIndex: i, 
      ...
      // Si no se encontraron secciones válidas, crear una sección genérica
      if (sections.length === 0) {
        this.log(LogLevel.WARN, 'No se encontraron secciones jerárquicas válidas, creando sección genérica');
        sections.push(this.createGenericSection(document, content));
      }

      return sections;
    } catch (error) {
      this.log(LogLevel.ERROR, 'Error general al extraer secciones jerárquicas', { error });
      return [this.createGenericSection(document, content)];
    }
  }

  /**
   * Extrae secciones por párrafos
   */
  private static extractParagraphSections(document: StoredDocument, config: ProcessingConfig): DocumentSection[] {
    ...
    for (let i = 0; i < paragraphs.length; i++) {
      ...
      // Crear nueva sección si alcanzamos el límite
    ...
    // Si no se encontraron secciones válidas, crear una sección genérica
    if (sections.length === 0) {
      this.log(LogLevel.WARN, 'No se encontraron párrafos válidos, creando sección genérica');
      sections.push(this.createGenericSection(document, content));
    }
  ...
  private static extractCustomSections(document: StoredDocument, config: ProcessingConfig): DocumentSection[] {
    ...
    try {
      ...
      // Usar patrones personalizados o patrón por defecto
      ...
      sections.push({
              ...
              this.log(LogLevel.ERROR, 'Error al procesar sección individual personalizada', { 
                sectionIndex: i, 
                pattern: patternStr,
                error: sectionError 
              });
          ...
          this.log(LogLevel.ERROR, 'Error al procesar patrón personalizado', { 
            pattern: patternStr,
            error: patternError 
          });
          // Continuar con el siguiente patrón en caso de error
          continue;
        }
      }
      ...
      // Si no se encontraron secciones válidas, crear una sección genérica
      if (sections.length === 0) {
        this.log(LogLevel.WARN, 'No se encontraron secciones personalizadas válidas, creando sección genérica');
        sections.push(this.createGenericSection(document, content));
      }
  ...
  /**
   * Extrae secciones genéricas del documento
   */
  private static extractGenericSections(document: StoredDocument, config: ProcessingConfig): DocumentSection[] {
    try {
      ...
      if (mainSections.length > 0) {
        ...
        // Si no hay secciones principales, procesar por párrafos
        const paragraphs = content.split(/\n\s*\n/);
        let currentSection: string[] = [];
        ...
        // Agregar la última sección si quedaron párrafos
        if (currentSection.length > 0) {
          const sectionContent = currentSection.join('\n\n');
          sections.push({
            id: `${document.id}-section-${order}`,
      ...
      // Si no se encontraron secciones, crear una sección genérica
      if (sections.length === 0) {
        this.log(LogLevel.WARN, 'No se encontraron secciones, creando sección genérica');
        sections.push(this.createGenericSection(document, content));
      }

      return sections;
    } catch (error) {
      this.log(LogLevel.ERROR, 'Error al extraer secciones genéricas', { error });
      return [this.createGenericSection(document, document.content)];
    }
  }

  /**
   * Determina el tipo de sección basado en el nivel jerárquico
   */
  ...
  /**
   * Crea una sección genérica para todo el contenido
   */
  ...
  /**
   * Genera un título para una sección basado en su contenido
   */
  private static generateSectionTitle(content: string, sectionNumber: number): string {
    // Intentar extraer la primera línea o frase como título
    ...
    // Si la primera línea es muy larga, extraer la primera frase
    const firstSentenceMatch = content.match(/^[^.!?]+[.!?]/);
    if (firstSentenceMatch && firstSentenceMatch[0].length <= 100) {
      return firstSentenceMatch[0].trim();
    }
    
    // Si todo falla, usar un título genérico
    return `Sección ${sectionNumber + 1}`;
  }

  /**
   * Obtiene la configuración de procesamiento para un documento
   */
  static getProcessingConfig(documentId: string): ProcessingConfig {
    try {
      ...
      this.log(LogLevel.WARN, 'Error al obtener configuración, usando valores por defecto', { error });
      return this.DEFAULT_CONFIG;
    }
  }

  /**
   * Guarda la configuración de procesamiento para un documento
   */
  static saveProcessingConfig(documentId: string, config: ProcessingConfig): void {
    try {
      ...
      this.log(LogLevel.INFO, 'Configuración guardada', { documentId });
    } catch (error) {
      this.log(LogLevel.ERROR, 'Error al guardar configuración', { error });
      throw new Error('No se pudo guardar la configuración');
    }
  }

  /**
   * Elimina la configuración de procesamiento de un documento
   */
  static deleteProcessingConfig(documentId: string): void {
    localStorage.removeItem(`${this.PROCESSING_CONFIG_KEY}_${documentId}`);
    this.log(LogLevel.INFO, 'Configuración eliminada', { documentId });
  }
  ...
  private static async getDocument(documentId: string): Promise<StoredDocument | null> {
    try {
      // Delegar la obtención del documento a StorageService
  ...
  /**
   * Extrae secciones de documentos militares con estructura específica
   * Maneja: CAPÍTULO X, secciones numeradas (1., 2., 3.), subsecciones (3.1.), párrafos numerados
   */
  private static extractMilitaryDoctrineSections(document: StoredDocument, config: ProcessingConfig): DocumentSection[] {
    try {
      ...
      // Patrones específicos para documentos militares
      const chapterPattern = /^CAPÍTULO\s+([IVX]+|\d+)\s+[A-ZÁÑÜ\s]+$/gm;
      const mainSectionPattern = /^\d+\.\s+[A-ZÁÑÜ\s]+$/gm;
      const subSectionPattern = /^\d+\.\d+\.\s+[A-Za-záñüÁÑÜ\s]+$/gm;
      
      // ✅ ACTUALIZADO: Patrón completo para artículos ordinales españoles (Primero hasta Nonagésimo)
      ...
      // ✅ NUEVO: Buscar artículos ordinales (Primero., Segundo., etc.)
      ...
      for (let i = 0; i < allMatches.length; i++) {
        ...
        try {
          ...
          // Validar que la sección tenga contenido suficiente
          if (sectionContent.length < (config.options.minSectionLength || 50)) {
            this.log(LogLevel.WARN, `Sección muy corta omitida: ${currentMatch.title}`);
            continue;
          }
          ...
          this.log(LogLevel.DEBUG, `Sección creada: ${currentMatch.title}`, {
            type: currentMatch.type,
            length: sectionContent.length
          });
          
        } catch (sectionError) {
          this.log(LogLevel.ERROR, `Error al procesar división: ${currentMatch.title}`, { error: sectionError });
          continue;
        }
      }

      // Si no se encontraron divisiones principales, procesar por párrafos numerados secuenciales
      if (sections.length === 0) {
        this.log(LogLevel.WARN, 'No se encontraron divisiones principales, procesando por párrafos numerados');
        return this.extractNumberedParagraphs(document, config);
      }

      this.log(LogLevel.INFO, `Extracción de doctrina militar completada: ${sections.length} secciones`);
      return sections;
  ...
  /**
   * Extrae secciones basándose en párrafos numerados secuencialmente
   * Para casos donde el documento usa numeración continua (1, 2, 3, ..., 147, 148, etc.)
   */
  private static extractNumberedParagraphs(document: StoredDocument, config: ProcessingConfig): DocumentSection[] {
    try {
      ...
      // Patrón para detectar párrafos numerados al inicio de línea
      ...
      // Agrupar párrafos en secciones lógicas
      ...
      for (let i = 0; i < paragraphs.length; i += sectionSize) {
        ...
        if (sectionContent.length >= (config.options.minSectionLength || 100)) {
          sections.push({
            ...
            documentId: document.id,
            title: `Párrafos ${startParagraph.number}-${endParagraph.number}`,
            content: sectionContent,
            type: SectionType.PARAGRAPH,
            order: order++,
            processed: false,
      ...
      this.log(LogLevel.ERROR, 'Error al extraer párrafos numerados', { error });
      return [this.createGenericSection(document, document.content)];
    }
  }

  /**
   * Detecta si un documento es de doctrina militar basándose en su contenido
   */
  static detectMilitaryDoctrine(content: string): boolean {
    ...
    const militaryIndicators = [
      ...
      /DEFENSA/gi,
      /MILITAR/gi,
      /ORGANIZACIÓN\s+DE\s+LAS\s+FAS/gi,
      /PLANEAMIENTO\s+DE\s+LA\s+DEFENSA/gi,
      /CAPACIDADES\s+MILITARES/gi,
      /TRANSFORMACIÓN\s+MILITAR/gi,
      // ✅ NUEVOS INDICADORES PARA DOCUMENTOS DE ORGANIZACIÓN MILITAR
      /ORGANIZACIÓN\s+DEL\s+EAYE/gi,
      /ESTADO\s+MAYOR/gi,
      /EJÉRCITO/gi,
      /ARMADA/gi,
      /AIRE/gi,
      ...
      /ESTRUCTURA\s+ORGÁNICA/gi,
      /ORGANIGRAMA/gi,
      /DEPENDENCIA\s+ORGÁNICA/gi,
      /UNIDADES\s+MILITARES/gi,
      // ✅ NUEVOS INDICADORES PARA INSTRUCCIONES MILITARES
      /INSTRUCCIÓN.*\d+-\d+/gi,
      /JEFE\s+DE\s+ESTADO\s+MAYOR.*EJÉRCITO\s+DEL\s+AIRE/gi,
      /EJÉRCITO\s+DEL\s+AIRE\s+Y\s+DEL\s+ESPACIO/gi,
      /(Primero|Segundo|Tercero|Cuarto|Quinto)\.\s+[A-ZÁÑÜ]/gi,
    ...
    // ✅ MEJORADO: Detección específica para documentos de organización militar española
    const hasEAYE = /EAYE|ESTADO\s+MAYOR\s+DEL\s+AIRE\s+Y\s+DEL\s+ESPACIO/gi.test(content);
    const hasOrganization = /ORGANIZACIÓN/gi.test(content);
    const hasMilitary = /MILITAR|EJÉRCITO|ARMADA|AIRE|DEFENSA/gi.test(content);
    const hasInstruction = /INSTRUCCIÓN.*\d+-\d+/gi.test(content) && /JEFE\s+DE\s+ESTADO\s+MAYOR.*EJÉRCITO\s+DEL\s+AIRE/gi.test(content);
    const hasOrdinalArticles = /(Primero|Segundo|Tercero|Cuarto|Quinto)\.\s+[A-ZÁÑÜ]/gi.test(content);
    ...
    // Si es documento de organización ET, instrucción militar, o tiene indicadores de organización militar, detectar como militar
    ...
    this.log(LogLevel.INFO, `Detección militar mejorada:`, {
  ...
  /**
   * Configura automáticamente el procesamiento para un documento de doctrina militar
   */
  ...
  /**
   * Procesa un documento detectando automáticamente si es PDC-01, militar general, o estándar
   */
  static async smartProcessDocument(document: StoredDocument): Promise<StoredDocument> {
    ...
    // ✅ DETECTAR Y GUARDAR CONFIGURACIÓN AUTOMÁTICAMENTE
    let configApplied = false;
    
    // Detectar primero si es PDC-01 específico
    if (this.detectPDC01Document(document.content)) {
      this.log(LogLevel.INFO, 'Documento PDC-01 B detectado automáticamente - Aplicando configuración');
      this.setupPDC01Processing(document.id);
      configApplied = true;
    }
    // Si no es PDC-01, verificar si es militar general
    else if (this.detectMilitaryDoctrine(document.content)) {
      this.log(LogLevel.INFO, 'Documento de doctrina militar detectado automáticamente - Aplicando configuración');
      this.setupMilitaryDoctrineProcessing(document.id);
      configApplied = true;
    }
    
    if (!configApplied) {
      this.log(LogLevel.INFO, 'Documento estándar detectado - Manteniendo configuración por defecto');
    }
    
    // Procesar con la configuración detectada o por defecto
    const processedDocument = await this.updateDocument(document);
    
    // 🆕 NUEVA FUNCIONALIDAD: Categorización automática
    try {
      this.log(LogLevel.INFO, 'Iniciando categorización automática de secciones');
      await CategorizationService.categorizeDocumentSections(document.id);
      this.log(LogLevel.INFO, 'Categorización automática completada');
    } catch (error) {
      this.log(LogLevel.ERROR, 'Error en categorización automática', { error: error.message });
    }
  ...
  /**
   * Lista de apartados específicos de PDC-01 B Doctrina para el empleo de las FAS
   * Organizados por capítulos con numeración jerárquica
   */
  private static PDC_01_APARTADOS_JERARQUICOS = {
    // CAPÍTULO 1 - ENTORNO ESTRATÉGICO
    'CAPITULO_1': [
    ...
    'CAPITULO_6': [
      'INTRODUCCIÓN',
      'ACCIONES MILITARES TÁCTICAS',
      'EL PLANEAMIENTO TÁCTICO DE LAS OPERACIONES',
      'LA EJECUCIÓN TÁCTICA DE LAS OPERACIONES'
    ],
    
    // CAPÍTULO 7 - MANDO Y CONTROL
    'CAPITULO_7': [
      'INTRODUCCIÓN',
  ...
  /**
   * Extrae secciones específicas para documentos PDC-01 B con numeración jerárquica
   */
  static extractPDC01Sections(document: StoredDocument, config: ProcessingConfig): DocumentSection[] {
    this.log(LogLevel.INFO, 'Procesando documento PDC-01 B con apartados específicos y numeración jerárquica');
    ...
    for (const [capituloKey, apartadosCapitulo] of Object.entries(this.PDC_01_APARTADOS_JERARQUICOS)) {
      ...
      for (const apartadoTitle of apartadosCapitulo) {
        // Buscar el apartado con múltiples patrones
        const patterns = [
          // Patrón con número al inicio: "1. ENTORNO GLOBAL DE SEGURIDAD"
          new RegExp(`^\\s*${numeroSeccion}\\.?\\s*${apartadoTitle.replace(/\s+/g, '\\s+')}`, 'gmi'),
          // Patrón sin número: "ENTORNO GLOBAL DE SEGURIDAD"
        ...
        for (let i = 0; i < patterns.length && !found; i++) {
          ...
          if (match) {
            this.log(LogLevel.DEBUG, `✅ Apartado encontrado: "${apartadoTitle}" con patrón ${i + 1}`);
            
            // Determinar dónde empieza y termina la sección
            const startPos = match.index;
            let endPos = content.length;
            
            // Buscar el siguiente apartado del mismo capítulo
            ...
            if (nextApartadoIndex < apartadosCapitulo.length) {
              ...
              // Si es el último apartado del capítulo, buscar el siguiente capítulo
              const nextCapituloPatterns = [
                /CAPÍTULO\s+\d+/gi,
                /^[IV]+\./gmi, // Números romanos
                /^\d+\.\s*[A-ZÁÑÜ]/gmi // Siguiente apartado numerado
              ];
        ...
        if (found && apartadoContent.length >= (config.options?.pdc01Doctrine?.minSectionLength || 200)) {
          // Crear título con numeración jerárquica
          ...
          this.log(LogLevel.DEBUG, `❌ Apartado NO encontrado: "${apartadoTitle}"`);
        }
        
        numeroSeccion++;
      }
      
      // Reset numeración para el siguiente capítulo
      numeroSeccion = 1;
    }
    
    this.log(LogLevel.INFO, `Encontrados ${apartados.length} apartados específicos PDC-01 con numeración jerárquica`);
    
    // Si no encuentra apartados específicos, intentar método militar general
    if (apartados.length === 0) {
      this.log(LogLevel.WARN, 'No se encontraron apartados específicos de PDC-01, usando método militar general');
      return this.extractMilitaryDoctrineSections(document, config);
    }
  ...
  /**
   * Detecta si un documento es específicamente PDC-01 B Doctrina para el empleo de las FAS
   * MEJORADO: Más flexible y preciso
   */
  static detectPDC01Document(content: string): boolean {
    // Indicadores específicos de PDC-01 (más flexibles y específicos)
    const pdc01Indicators = [
      /PDC[.\-\s]*01/gi,
      /Doctrina\s+para\s+el\s+empleo\s+de\s+las\s+FAS/gi,
      /ENTORNO\s+GLOBAL\s+DE\s+SEGURIDAD/gi,
      /CONTEXTO\s+DE\s+COMPETICIÓN/gi,
    ...
    // ✅ MEJORADO: Reducir aún más el umbral y agregar verificación específica
    ...
    this.log(LogLevel.INFO, `PDC-01 detección mejorada:`, {
      indicadores: indicatorCount,
  ...
  /**
   * Configura automáticamente el procesamiento para un documento PDC-01
   */
