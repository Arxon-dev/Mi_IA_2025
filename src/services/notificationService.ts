import { TelegramService } from './telegramService';
import { prisma } from '../lib/prisma';
// Evitar dependencia circular - usar lazy loading
// import { IntelligentAlertsService } from './intelligentAlertsService';

// Define NotificationEventType locally since it's not generated by Prisma
type NotificationEventType = 
  | 'FIRST_GRADUATION_TODAY'
  | 'MILESTONE_5_GRADUATIONS'
  | 'MILESTONE_10_GRADUATIONS'
  | 'MILESTONE_25_GRADUATIONS'
  | 'MILESTONE_50_GRADUATIONS'
  | 'MILESTONE_100_GRADUATIONS'
  | 'INACTIVITY_REMINDER'
  | 'WEEKLY_PROGRESS_REPORT'
  | 'PERFORMANCE_DROP'
  | 'INACTIVITY_WARNING'
  | 'RESPONSE_TIME_INCREASE'
  | 'STREAK_BREAK_RISK'
  | 'DIFFICULTY_REGRESSION'
  | 'MOTIVATIONAL_BOOST'
  | 'STUDY_FREQUENCY_DROP';

interface NotificationEvent {
  id: string;
  userid: string;
  type: NotificationEventType;
  eventData: any;
  triggerContext: any;
  scheduledFor?: Date;
}

interface NotificationResult {
  success: boolean;
  method: string;
  message: string;
}

/**
 * 🔔 Servicio de Notificaciones Inteligentes
 * 
 * Maneja notificaciones contextuales y gamificadas para usuarios
 * con configuraciones personalizables y múltiples canales de entrega.
 */
export class NotificationService {
  private telegramBot: TelegramService | null = null;
  private intelligentAlertsService: any = null; // Lazy loading

  constructor() {
    // Inicialización básica - IntelligentAlertsService se cargará cuando sea necesario
  }

  /**
   * 🔄 Obtener instancia de IntelligentAlertsService con lazy loading
   */
  private async getIntelligentAlertsService() {
    if (!this.intelligentAlertsService) {
      const { IntelligentAlertsService } = await import('./intelligentAlertsService');
      this.intelligentAlertsService = new IntelligentAlertsService();
    }
    return this.intelligentAlertsService;
  }

  /**
   * 🤖 Configurar bot de Telegram
   */
  setTelegramBot(bot: TelegramService): void {
    this.telegramBot = bot;
  }

  // ============================================
  // 🎯 NOTIFICACIONES DE QUIZ/RESPUESTAS
  // ============================================

  /**
   * 📤 Enviar respuesta de quiz a usuario o grupo
   */
  static async sendQuizResponse(
    telegramUserId: string,
    message: string,
    options: { 
      parseMode?: 'HTML' | 'Markdown';
      groupChatId?: string;
      userName?: string;
    } = {}
  ): Promise<NotificationResult> {
    const BOT_TOKEN = process.env.TELEGRAM_BOT_TOKEN;
    const GROUP_CHAT_ID = process.env.TELEGRAM_GROUP_CHAT_ID;
    
    if (!BOT_TOKEN) {
    return {
      success: false,
        method: 'no_token',
        message: 'Token de bot no configurado'
      };
    }

    const { parseMode = 'HTML', groupChatId = GROUP_CHAT_ID, userName = 'Usuario' } = options;

    // 1. Intentar envío directo al usuario
    try {
      const directResponse = await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          chat_id: telegramUserId,
          text: message,
          parse_mode: parseMode
        })
      });
      
      if (directResponse.ok) {
        return {
          success: true,
          method: 'direct',
          message: `Respuesta enviada directamente a ${userName}`
        };
      }
    } catch (error) {
      console.log(`⚠️ Envío directo falló para ${userName}, intentando grupo...`);
    }

    // 2. Fallback: enviar al grupo con mención
    if (groupChatId) {
      try {
        const groupMessage = `🎯 <b>Respuesta para ${userName}</b>\n\n${message}`;
        
        const groupResponse = await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/sendMessage`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
            chat_id: groupChatId,
            text: groupMessage,
            parse_mode: parseMode
        })
      });
      
        if (groupResponse.ok) {
        return {
          success: true,
            method: 'group_fallback',
            message: `Respuesta de quiz enviada al grupo como fallback para ${userName}`
        };
      }
    } catch (error) {
        console.error('❌ Error enviando al grupo:', error);
      }
    }

    // 3. Ambos métodos fallaron
    if (groupChatId) {
      return {
        success: true,
        method: 'group_fallback',
        message: `Respuesta de quiz enviada al grupo como fallback para ${userName}`
      };
    } else {
      return {
        success: false,
        method: 'failed',
        message: `No se pudo enviar respuesta de quiz a ${userName}`
      };
    }
  }

  /**
   * Reemplazar placeholders en templates
   */
  private static replaceTemplate(template: string, data: Record<string, any>): string {
    let result = template;
    
    Object.entries(data).forEach(([key, value]) => {
      const placeholder = `{{${key}}}`;
      result = result.replace(new RegExp(placeholder, 'g'), String(value));
    });
    
    return result;
  }

  // ============================================
  // 🎯 GESTIÓN DE EVENTOS DE NOTIFICACIÓN
  // ============================================

  /**
   * 📝 Crear nuevo evento de notificación
   */
  private async createNotificationEvent(eventData: {
    userid: string;
    type: NotificationEventType;
    eventData: any;
    triggerContext: any;
    scheduledFor?: Date;
  }): Promise<void> {
    try {
      await prisma.notificationevent.create({
        data: {
          id: this.generateId(),
          userid: eventData.userid,
          type: eventData.type,
          eventdata: JSON.stringify(eventData.eventData),
          triggercontext: JSON.stringify(eventData.triggerContext),
          scheduledfor: eventData.scheduledFor,
          createdat: new Date(),
          updatedat: new Date()
        }
      });
    } catch (error) {
      console.error('❌ Error creando evento de notificación:', error);
    }
  }

  /**
   * 🔄 Procesar notificaciones programadas
   */
  async processScheduledNotifications(): Promise<void> {
    try {
      const now = new Date();
      
      // Obtener notificaciones pendientes
      const pendingNotifications = await prisma.notificationevent.findMany({
        where: {
          sent: false,
          scheduledfor: {
            lte: now
          }
        },
        orderBy: {
          scheduledfor: 'asc'
        },
        take: 10 // Procesar máximo 10 por vez
      });

      console.log(`📬 Procesando ${pendingNotifications.length} notificaciones programadas`);

      for (const notification of pendingNotifications) {
        await this.sendNotificationToUser(notification);
      }

    } catch (error) {
      console.error('❌ Error procesando notificaciones programadas:', error);
    }
  }

  /**
   * 📊 Obtener resumen semanal de un usuario
   */
  async generateWeeklyReport(userid: string): Promise<any> {
    try {
      const weekAgo = new Date();
      weekAgo.setDate(weekAgo.getDate() - 7);

      // Obtener estadísticas de la semana
      const weeklyStats = await prisma.studyresponse.groupBy({
        by: ['subject'],
        where: {
          userid,
          answeredat: {
            gte: weekAgo
          }
        },
        _count: {
          id: true
        },
        _sum: {
          responsetime: true
        }
      });

      // Calcular graduaciones de la semana
      const graduationsThisWeek = await this.getTotalGraduationsCount(userid);

      return {
        weeklyStats,
        graduationsThisWeek,
        totalQuestions: weeklyStats.reduce((sum, stat) => sum + stat._count.id, 0),
        averageResponseTime: weeklyStats.reduce((sum, stat) => sum + (stat._sum.responsetime || 0), 0) / weeklyStats.length
      };

    } catch (error) {
      console.error('❌ Error generando reporte semanal:', error);
      return null;
    }
  }

  /**
   * 📅 Verificar y crear reportes semanales
   */
  async checkWeeklyReports(): Promise<void> {
    try {
      const today = new Date();
      const dayOfWeek = today.getDay(); // 0 = domingo, 1 = lunes, etc.
      
      // Solo generar reportes los lunes
      if (dayOfWeek !== 1) return;

      // Buscar usuarios activos en la última semana
      const weekAgo = new Date();
      weekAgo.setDate(weekAgo.getDate() - 7);

      const activeUsers = await prisma.telegramuser.findMany({
        where: {
          lastactivity: {
            gte: weekAgo
          }
        },
        // Note: Removing notificationSettings include since it doesn't exist in schema
      });

      for (const user of activeUsers) {
        // Verificar si ya se envió reporte esta semana
        const startOfWeek = new Date(today);
        startOfWeek.setDate(today.getDate() - today.getDay() + 1); // Lunes de esta semana

        const existingReport = await prisma.notificationevent.findFirst({
          where: {
            userid: user.id,
            type: 'WEEKLY_PROGRESS_REPORT',
            createdat: {
              gte: startOfWeek
            }
          }
        });

        if (!existingReport) {
          const weeklyReport = await this.generateWeeklyReport(user.id);
          
          if (weeklyReport) {
            await this.createNotificationEvent({
              userid: user.id,
              type: 'WEEKLY_PROGRESS_REPORT',
              eventData: weeklyReport,
              triggerContext: { weekOf: startOfWeek }
            });
          }
        }
      }

    } catch (error) {
      console.error('❌ Error verificando reportes semanales:', error);
    }
  }

  // ============================================
  // 🎓 DETECTORES DE EVENTOS
  // ============================================

  /**
   * 🎓 Detectar primera graduación del día
   */
  async checkFirstGraduationToday(userid: string, subject: string): Promise<boolean> {
    try {
      const today = new Date();
      today.setHours(0, 0, 0, 0);
      const tomorrow = new Date(today);
      tomorrow.setDate(tomorrow.getDate() + 1);

      // Verificar si es la primera graduación del día
      const todayGraduations = await prisma.notificationevent.count({
        where: {
          userid,
          type: 'FIRST_GRADUATION_TODAY',
          createdat: {
            gte: today,
            lt: tomorrow
          }
        }
      });

      if (todayGraduations === 0) {
        await this.createNotificationEvent({
          userid,
          type: 'FIRST_GRADUATION_TODAY',
          eventData: { subject, timestamp: new Date() },
          triggerContext: { graduationSubject: subject }
        });
        return true;
      }

      return false;
    } catch (error) {
      console.error('❌ Error verificando primera graduación del día:', error);
      return false;
    }
  }

  /**
   * 🏆 Detectar milestone de graduaciones (5, 10, 25, 50, 100)
   */
  async checkGraduationMilestones(userid: string): Promise<NotificationEventType | null> {
    try {
      // Contar graduaciones históricas del usuario
      const totalGraduations = await this.getTotalGraduationsCount(userid);
      
      const milestones = [
        { count: 5, type: 'MILESTONE_5_GRADUATIONS' as NotificationEventType },
        { count: 10, type: 'MILESTONE_10_GRADUATIONS' as NotificationEventType },
        { count: 25, type: 'MILESTONE_25_GRADUATIONS' as NotificationEventType },
        { count: 50, type: 'MILESTONE_50_GRADUATIONS' as NotificationEventType },
        { count: 100, type: 'MILESTONE_100_GRADUATIONS' as NotificationEventType }
      ];

      for (const milestone of milestones) {
        if (totalGraduations === milestone.count) {
          // Verificar que no se haya enviado ya este milestone
          const existingNotification = await prisma.notificationevent.findFirst({
            where: {
              userid,
              type: milestone.type,
              sent: true
            }
          });

          if (!existingNotification) {
            await this.createNotificationEvent({
              userid,
              type: milestone.type,
              eventData: { totalGraduations, milestone: milestone.count },
              triggerContext: { achievementLevel: milestone.count }
            });
            return milestone.type;
          }
        }
      }

      return null;
    } catch (error) {
      console.error('❌ Error verificando milestones de graduación:', error);
      return null;
    }
  }

  /**
   * ⏰ Verificar usuarios inactivos para recordatorios
   */
  async checkInactiveUsers(): Promise<string[]> {
    try {
      const threeDaysAgo = new Date();
      threeDaysAgo.setDate(threeDaysAgo.getDate() - 3);

      // Buscar usuarios sin actividad en estudio de falladas en los últimos 3 días
      const inactiveUsers = await prisma.telegramuser.findMany({
        where: {
          lastactivity: {
            lt: threeDaysAgo
          }
        }
      });

      const usersToNotify: string[] = [];

      for (const user of inactiveUsers) {
        // For now, notify all inactive users since we don't have notification settings
        // In the future, this can be enhanced with user preferences

        await this.createNotificationEvent({
          userid: user.id,
          type: 'INACTIVITY_REMINDER',
          eventData: { daysSinceLastActivity: Math.floor((Date.now() - user.lastactivity.getTime()) / (1000 * 60 * 60 * 24)) },
          triggerContext: { lastActivity: user.lastactivity }
        });

        usersToNotify.push(user.id);
      }

      return usersToNotify;
    } catch (error) {
      console.error('❌ Error verificando usuarios inactivos:', error);
      return [];
    }
  }

  // ============================================
  // 📧 TEMPLATES DE MENSAJES
  // ============================================

  /**
   * 📝 Generar mensaje de notificación según el tipo
   */
  private generateNotificationMessage(type: NotificationEventType, eventData: any, userName: string): string {
    switch (type) {
      case 'FIRST_GRADUATION_TODAY':
        return `🎓 <b>¡Primera graduación del día!</b>

¡Felicidades ${userName}! 🎉

Has graduado tu primera pregunta de <b>${eventData.subject}</b> hoy. ¡Un excelente comienzo!

🎯 <b>MANTÉN EL IMPULSO:</b>
• <code>/falladas10</code> - 10 preguntas falladas más
• <code>/${eventData.subject}10</code> - Estudiar más de ${eventData.subject}
• <code>/miprogreso</code> - Ver tu evolución

🏆 <b>COMPITE Y MEJORA:</b>
• <code>/duelo</code> - Reto a otro usuario
• <code>/ranking</code> - Ve tu posición actual

🔥 <i>¡La primera graduación del día es el mejor arranque!</i>`;

      case 'MILESTONE_5_GRADUATIONS':
        return `🏆 <b>¡5 Graduaciones Alcanzadas!</b>

¡Increíble ${userName}! 🚀

Has graduado <b>5 preguntas</b> con éxito. Tu constancia está dando frutos.

📈 <b>ACELERA HACIA LAS 10:</b>
• <code>/falladas20</code> - Sesión intensiva de 20
• <code>/estadisticas</code> - Ve tus materias más débiles
• <code>/simulacro</code> - Pon a prueba tus conocimientos

🎯 <b>COMPARTE TU PROGRESO:</b>
• <code>/logros</code> - Ve todos tus achievements
• <code>/ranking</code> - Compara tu posición

💡 <i>¡Solo necesitas 5 graduaciones más para el siguiente milestone!</i>`;

      case 'MILESTONE_10_GRADUATIONS':
        return `🎖️ <b>¡10 Graduaciones Completadas!</b>

¡Excelente trabajo ${userName}! 🌟

Has alcanzado las <b>10 graduaciones</b>. Estás construyendo una base sólida.

🚀 <b>DESBLOQUEA NUEVAS FUNCIONES:</b>
• <code>/duelo</code> - Ahora puedes retar a otros usuarios
• <code>/torneo</code> - Participa en torneos semanales
• <code>/estadisticas_avanzadas</code> - Análisis detallado

🎯 <b>ACELERA TU PROGRESO:</b>
• <code>/falladas30</code> - Sesión intensiva
• <code>/simulacro_completo</code> - Examen de 100 preguntas

🏆 <i>¡Siguiente objetivo: 25 graduaciones!</i>`;

      case 'MILESTONE_25_GRADUATIONS':
        return `🏅 <b>¡25 Graduaciones Dominadas!</b>

¡Impresionante ${userName}! ⭐

Has graduado <b>25 preguntas</b>. Estás en el camino hacia la maestría.

🎓 <b>NIVEL AVANZADO DESBLOQUEADO:</b>
• <code>/mentor</code> - Ayuda a otros usuarios
• <code>/crear_duelo_personalizado</code> - Crea retos únicos
• <code>/analisis_debilidades</code> - Identifica áreas de mejora

💪 <b>RETOS DE ÉLITE:</b>
• <code>/simulacro_cronometrado</code> - Prueba contra el tiempo
• <code>/torneo_avanzado</code> - Competición de alto nivel

🌟 <i>¡Siguiente meta: 50 graduaciones de élite!</i>`;

      case 'MILESTONE_50_GRADUATIONS':
        return `👑 <b>¡50 Graduaciones de Élite!</b>

¡Extraordinario ${userName}! 🏆

Has graduado <b>50 preguntas</b>. Te has convertido en un verdadero experto.

🎯 <b>FUNCIONES EXCLUSIVAS:</b>
• <code>/crear_torneo</code> - Organiza tus propios torneos
• <code>/coaching_personalizado</code> - Sesiones avanzadas
• <code>/estadisticas_comparativas</code> - Compara con los mejores

🚀 <b>RETOS LEGENDARIOS:</b>
• <code>/simulacro_maestria</code> - Prueba de conocimiento total
• <code>/liga_profesional</code> - Competición entre expertos

👑 <i>¡Objetivo final: 100 graduaciones legendarias!</i>`;

      case 'MILESTONE_100_GRADUATIONS':
        return `🏆 <b>¡100 GRADUACIONES LEGENDARIAS!</b>

¡INCREÍBLE ${userName}! 🎉🎊

Has alcanzado las <b>100 graduaciones</b>. ¡Eres una LEYENDA!

👑 <b>ESTATUS LEGENDARIO DESBLOQUEADO:</b>
• <code>/mentor_elite</code> - Guía a la nueva generación
• <code>/crear_contenido</code> - Contribuye con preguntas
• <code>/ranking_leyendas</code> - Hall of Fame exclusivo

🌟 <b>RECONOCIMIENTO ESPECIAL:</b>
• Insignia dorada permanente
• Acceso a funciones beta
• Mención en el canal oficial

🎯 <i>¡Continúa dominando y mantén tu estatus legendario!</i>`;

      case 'INACTIVITY_REMINDER':
        return `⏰ <b>¡Te echamos de menos!</b>

Hola ${userName} 👋

Hace <b>${eventData.daysSinceLastActivity} días</b> que no practicas. ¡Tu progreso te está esperando!

🎯 <b>RETOMA TU ENTRENAMIENTO:</b>
• <code>/falladas5</code> - Sesión rápida de repaso
• <code>/miprogreso</code> - Ve tu estado actual
• <code>/estadisticas</code> - Revisa tus fortalezas

💪 <b>MANTÉN TU RACHA:</b>
• <code>/ranking</code> - Ve tu posición
• <code>/duelo</code> - Reta a un amigo
• <code>/torneo</code> - Únete al torneo activo

🔥 <i>¡Cada día cuenta para tu éxito!</i>`;

      case 'WEEKLY_PROGRESS_REPORT':
        return `📊 <b>Reporte Semanal de Progreso</b>

¡Hola ${userName}! 📈

Aquí tienes tu resumen de la semana:

📚 <b>ACTIVIDAD SEMANAL:</b>
• Preguntas respondidas: ${eventData.totalQuestions}
• Tiempo promedio: ${Math.round(eventData.averageResponseTime / 1000)}s
• Graduaciones nuevas: ${eventData.graduationsThisWeek}

🎯 <b>PARA ESTA SEMANA:</b>
• <code>/falladas15</code> - Sesión de repaso
• <code>/simulacro</code> - Pon a prueba tus conocimientos
• <code>/duelo</code> - Compite con otros usuarios

💡 <i>¡Mantén el ritmo y sigue mejorando!</i>`;

      // Alertas inteligentes
      case 'PERFORMANCE_DROP':
      case 'INACTIVITY_WARNING':
      case 'RESPONSE_TIME_INCREASE':
      case 'STREAK_BREAK_RISK':
      case 'DIFFICULTY_REGRESSION':
      case 'MOTIVATIONAL_BOOST':
      case 'STUDY_FREQUENCY_DROP':
        // Para alertas inteligentes, el mensaje ya viene generado desde el servicio
        return eventData.message || `🔔 Alerta inteligente para ${userName}`;

      default:
        return `🔔 Notificación de ${userName}`;
    }
  }

  // ============================================
  // 📤 ENVÍO DE NOTIFICACIONES
  // ============================================

  /**
   * 📤 Enviar notificación individual a un usuario
   */
  private async sendNotificationToUser(notification: any): Promise<boolean> {
    try {
      // Get user info
      const user = await prisma.telegramuser.findUnique({
        where: { id: notification.userid }
      });

      if (!user) {
        console.error(`❌ Usuario no encontrado: ${notification.userid}`);
        return false;
      }

      // For now, send all notifications since we don't have notification settings
      // In the future, this can be enhanced with user preferences
      
      // Verificar horario permitido (basic implementation)
      const currentHour = new Date().getHours();
      const startHour = 8;  // Default start hour
      const endHour = 22;   // Default end hour
      
      if (currentHour < startHour || currentHour >= endHour) {
        console.log(`⏰ Notificación ${notification.type} pospuesta (fuera de horario)`);
        // Reprogramar para el próximo período válido
        const tomorrow = new Date();
        tomorrow.setDate(tomorrow.getDate() + 1);
        tomorrow.setHours(startHour, 0, 0, 0);
        
        await prisma.notificationevent.update({
          where: { id: notification.id },
          data: { scheduledfor: tomorrow }
        });
        return false;
      }

      // Generar mensaje
      const userName = user.firstname || user.username || 'Usuario';
      const message = this.generateNotificationMessage(
        notification.type,
        JSON.parse(notification.eventdata || '{}'),
        userName
      );

      // Enviar mensaje por Telegram
      let telegramMsgId: string | null = null;
      if (this.telegramBot) {
        const result = await this.telegramBot.sendMessage(
          user.telegramuserid,
          message
        );
        telegramMsgId = result.message_id?.toString();
      }

      // Marcar como enviada
      await this.markNotificationAsSent(
        notification.id,
        true,
        null,
        message,
        telegramMsgId
      );

      // Actualizar tiempo de última notificación
      await this.updateLastNotificationTime(notification.userid, notification.type);

      console.log(`✅ Notificación ${notification.type} enviada a ${userName}`);
      return true;

    } catch (error) {
      console.error('❌ Error enviando notificación:', error);
      
      // Marcar como fallida
      await this.markNotificationAsSent(
        notification.id,
        false,
        error instanceof Error ? error.message : 'Error desconocido',
        undefined,
        undefined
      );

      return false;
    }
  }

  /**
   * ✅ Marcar notificación como enviada
   */
  private async markNotificationAsSent(
    notificationId: string, 
    success: boolean, 
    errorMessage?: string | null,
    messageContent?: string,
    telegramMsgId?: string | null
  ): Promise<void> {
    await prisma.notificationevent.update({
      where: { id: notificationId },
      data: {
        sent: true,
        sentat: new Date(),
        success,
        errormessage: errorMessage,
        messagecontent: messageContent,
        telegrammsgid: telegramMsgId,
        updatedat: new Date()
      }
    });
  }

  /**
   * 🔍 Verificar si se debe enviar la notificación según configuraciones
   */
  private shouldSendNotification(type: NotificationEventType, settings: any): boolean {
    if (!settings) return true; // Si no hay configuraciones, enviar por defecto

    switch (type) {
      case 'FIRST_GRADUATION_TODAY':
        return settings.graduationNotifications !== false;
      case 'MILESTONE_5_GRADUATIONS':
      case 'MILESTONE_10_GRADUATIONS':
      case 'MILESTONE_25_GRADUATIONS':
      case 'MILESTONE_50_GRADUATIONS':
      case 'MILESTONE_100_GRADUATIONS':
        return settings.milestoneNotifications !== false;
      case 'INACTIVITY_REMINDER':
        return settings.reminderNotifications !== false;
      case 'WEEKLY_PROGRESS_REPORT':
        return settings.weeklyReportNotifications !== false;
      default:
        return true;
    }
  }

  /**
   * 📅 Actualizar tiempo de última notificación en configuraciones
   */
  private async updateLastNotificationTime(userid: string, type: NotificationEventType): Promise<void> {
    // For now, this is a no-op since we don't have user notification settings table
    // In the future, this can be implemented when user preferences are added
    console.log(`📅 Actualización de tiempo de notificación para ${userid}: ${type}`);
  }

  // ============================================
  // 📊 UTILIDADES
  // ============================================

  /**
   * 📊 Obtener total de graduaciones de un usuario
   */
  private async getTotalGraduationsCount(userid: string): Promise<number> {
    try {
      // Contar preguntas graduadas (1+ aciertos desde último fallo)
      // 🔧 FIX: Usar sintaxis MySQL correcta con parámetros posicionales
      const result = await prisma.$queryRawUnsafe(`
        SELECT COUNT(DISTINCT fq.questionid) as count
        FROM (
          SELECT 
            sr.questionid,
            sr.subject,
            COALESCE(
              (SELECT COUNT(*) 
               FROM studyresponse sr2 
               WHERE sr2.userid = sr.userid 
                 AND sr2.questionid = sr.questionid 
                 AND sr2.subject = sr.subject
                 AND sr2.iscorrect = true 
                 AND sr2.answeredat > COALESCE(
                   (SELECT MAX(sr3.answeredat) 
                    FROM studyresponse sr3 
                    WHERE sr3.userid = sr.userid 
                      AND sr3.questionid = sr.questionid 
                      AND sr3.subject = sr.subject
                      AND sr3.iscorrect = false), 
                   '1970-01-01')
              ), 0) as total_successes_since_last_fail
          FROM studyresponse sr
          WHERE sr.userid = ?
            AND sr.iscorrect = false
          GROUP BY sr.questionid, sr.subject, sr.userid
        ) fq
        WHERE fq.total_successes_since_last_fail >= 1
      `, userid);

      return Number((result as any[])[0]?.count || 0);
    } catch (error) {
      console.error('❌ Error contando graduaciones totales:', error);
      return 0;
    }
  }

  /**
   * 🔧 Obtener configuraciones de notificación de un usuario
   */
  async getUserNotificationSettings(userid: string): Promise<any> {
    // For now, return default settings since we don't have the table
    // In the future, this can be implemented when user preferences are added
    return {
      graduationNotifications: true,
      milestoneNotifications: true,
      reminderNotifications: true,
      weeklyReportNotifications: true,
      notificationStartHour: 8,
      notificationEndHour: 22
    };
  }

  /**
   * 🔧 Generar ID único
   */
  private generateId(): string {
    return Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);
  }

  /**
   * ⚙️ Actualizar configuraciones de notificación
   */
  async updateUserNotificationSettings(userid: string, settings: any): Promise<void> {
    // For now, this is a no-op since we don't have user notification settings table
    // In the future, this can be implemented when user preferences are added
    console.log(`⚙️ Actualización de configuraciones para ${userid}:`, settings);
  }

  /**
   * 🧪 Verificar si el usuario puede recibir mensajes privados
   */
  static async canReceivePrivateMessages(telegramuserid: string): Promise<boolean> {
    try {
      const BOT_TOKEN = process.env.TELEGRAM_BOT_TOKEN;
      if (!BOT_TOKEN) return false;
      
      // Intentar obtener info del chat privado
      const response = await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/getChat`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          chat_id: telegramuserid
        })
      });

      const result = await response.json() as any;
      return result.ok;
      
    } catch (error) {
      return false;
    }
  }

  /**
   * 📤 Enviar respuesta inteligente de quiz
   */
  static async sendIntelligentQuizResponse(
    userStats: any,
    iscorrect: boolean,
    questionData: any,
    user: { telegramuserid: string; firstName?: string; firstname?: string; username?: string },
    groupChatId: string
  ): Promise<{ success: boolean; method: string; message: string }> {
    
    const userName = user.firstName || user.firstname || user.username || 'Usuario';
    
    // Formatear mensaje de respuesta
    const feedback = iscorrect ? '✅ ¡Correcto!' : '❌ Incorrecto';
    const message = `${feedback} ${userName}`;
    
    // Usar el método existente sendQuizResponse
    return await this.sendQuizResponse(user.telegramuserid, message, {
      parseMode: 'HTML',
      groupChatId,
      userName
    });
  }

  /**
   * 📤 Enviar respuesta inteligente de comando
   */
  static async sendIntelligentCommandResponse(
    originalCommand: string,
    response: string,
    user: { telegramuserid: string; firstName?: string; firstname?: string },
    groupChatId: string,
    isGroupChat: boolean
  ): Promise<{ success: boolean; method: string; message: string }> {
    
    const userName = user.firstName || user.firstname || 'Usuario';
    
    // Para comandos en grupo, usar el método existente
    return await this.sendQuizResponse(user.telegramuserid, response, {
      parseMode: 'HTML',
      groupChatId,
      userName
    });
  }

  /**
   * 📤 Enviar notificación inteligente
   */
  static async sendIntelligentNotification(
    type: 'duel' | 'achievement' | 'goal',
    targetUser: { telegramuserid: string; firstName?: string; firstname?: string },
    data: any,
    groupChatId: string
  ): Promise<{ success: boolean; method: string; message: string }> {
    
    const userName = targetUser.firstName || targetUser.firstname || 'Usuario';
    
    // Generar mensaje según el tipo
    let message = '';
    switch (type) {
      case 'duel':
        message = `🗡️ ${userName}, tienes un nuevo duelo pendiente!`;
        break;
      case 'achievement':
        message = `🏆 ${userName}, has desbloqueado un nuevo logro!`;
        break;
      case 'goal':
        message = `🎯 ${userName}, has alcanzado una nueva meta!`;
        break;
    }
    
    // Usar el método existente
    return await this.sendQuizResponse(targetUser.telegramuserid, message, {
      parseMode: 'HTML',
      groupChatId,
      userName
    });
  }

  // ============================================
  // 🎯 MÉTODOS PÚBLICOS PARA INTEGRACIÓN
  // ============================================

  /**
   * 🎓 Llamar cuando una pregunta se gradúa
   */
  async onQuestionGraduated(userid: string, subject: string): Promise<void> {
    await this.checkFirstGraduationToday(userid, subject);
    await this.checkGraduationMilestones(userid);
  }

  /**
   * ⏰ Ejecutar verificaciones programadas (cron job)
   */
  async runScheduledChecks(): Promise<void> {
    console.log('🔍 Ejecutando verificaciones programadas de notificaciones...');
    
    await this.checkInactiveUsers();
    await this.checkWeeklyReports();
    await this.processScheduledNotifications();
    
    // Ejecutar alertas inteligentes
    await this.runIntelligentAlerts();
    
    console.log('✅ Verificaciones programadas completadas');
  }

  /**
   * 🧠 Ejecutar sistema de alertas inteligentes
   */
  async runIntelligentAlerts(): Promise<{
    usersProcessed: number;
    alertsSent: number;
    errors: number;
    alertsByType?: Record<string, number>;
  }> {
    try {
      console.log('🧠 Ejecutando alertas inteligentes...');
      const intelligentAlertsService = await this.getIntelligentAlertsService();
      const result = await intelligentAlertsService.processAlertsForAllUsers();
      console.log('✅ Alertas inteligentes completadas');
      return result;
    } catch (error) {
      console.error('❌ Error ejecutando alertas inteligentes:', error);
      return {
        usersProcessed: 0,
        alertsSent: 0,
        errors: 1,
        alertsByType: {}
      };
    }
  }

  /**
   * 📊 Verificar alertas para un usuario específico
   */
  async checkUserAlerts(userId: string): Promise<Array<{
    type: string;
    message: string;
    timestamp: Date;
  }>> {
    try {
      const intelligentAlertsService = await this.getIntelligentAlertsService();
      const alerts = await intelligentAlertsService.checkForAlerts(userId);
      
      for (const alert of alerts) {
        await intelligentAlertsService.sendAlert(alert);
      }
      
      return alerts.map(alert => ({
        type: alert.type,
        message: alert.message,
        timestamp: new Date()
      }));
    } catch (error) {
      console.error(`❌ Error verificando alertas para usuario ${userId}:`, error);
      return [];
    }
  }

  /**
   * 📈 Obtener métricas de rendimiento de un usuario
   */
  async getUserPerformanceMetrics(userId: string, days: number = 7): Promise<{
    accuracyTrend: number;
    studyFrequency: number;
    averageResponseTime: number;
    currentStreak: number;
    difficultyProgression: number;
    engagementScore: number;
  }> {
    try {
      const intelligentAlertsService = await this.getIntelligentAlertsService();
      const metrics = await intelligentAlertsService.calculatePerformanceMetrics(userId, days);
      return metrics || {
        accuracyTrend: 0,
        studyFrequency: 0,
        averageResponseTime: 0,
        currentStreak: 0,
        difficultyProgression: 0,
        engagementScore: 0
      };
    } catch (error) {
      console.error(`❌ Error obteniendo métricas para usuario ${userId}:`, error);
      return {
        accuracyTrend: 0,
        studyFrequency: 0,
        averageResponseTime: 0,
        currentStreak: 0,
        difficultyProgression: 0,
        engagementScore: 0
      };
    }
  }

  /**
   * 📊 Obtener estadísticas del sistema de alertas
   */
  async getIntelligentAlertsStats(): Promise<{
    alertsSentToday: number;
    activeUsersMonitored: number;
    lastExecution: string | null;
    alertsByType?: Record<string, number>;
  }> {
    try {
      const intelligentAlertsService = await this.getIntelligentAlertsService();
      const stats = await intelligentAlertsService.getAlertStats();
      return stats || {
        alertsSentToday: 0,
        activeUsersMonitored: 0,
        lastExecution: null,
        alertsByType: {}
      };
    } catch (error) {
      console.error('❌ Error obteniendo estadísticas de alertas:', error);
      return {
        alertsSentToday: 0,
        activeUsersMonitored: 0,
        lastExecution: null,
        alertsByType: {}
      };
    }
  }
}

export const notificationService = new NotificationService();
export default notificationService;