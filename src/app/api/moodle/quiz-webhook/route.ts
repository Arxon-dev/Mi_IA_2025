...
// ==========================================
// 🎯 WEBHOOK PARA ACTIVIDADES DE QUIZ MOODLE
// ==========================================

export async function POST(request: NextRequest) {
  console.log('🎯 WEBHOOK MOODLE: Recibiendo datos del observer');
  
  try {
    const body = await request.json();
    
    // ✅ LOGGING COMPLETO DE DATOS RECIBIDOS
    ...
    if (body.quizData) {
      console.log('📊 Quiz Data Keys:', Object.keys(body.quizData));
      console.log('📊 Quiz Data Content:', JSON.stringify(body.quizData, null, 2));
    }
    
    if (body.questions && Array.isArray(body.questions)) {
      console.log(`📝 Total preguntas recibidas: ${body.questions.length}`);
    ...
    // ✅ MANEJAR AMBOS FORMATOS: con wrapper y directo del observer
    let quizData, questions;
    
    if (body.quizData) {
      // Formato con wrapper (quiz simulado)
      console.log('📦 Formato con quizData wrapper detectado');
      quizData = body.quizData;
      questions = body.questions || [];
    } else if (body.moodleUserId) {
      // Formato directo del observer de Moodle  
      console.log('📦 Formato directo del observer detectado');
      quizData = {
      ...
      console.error('❌ Error: Formato de datos no reconocido');
      console.log('📦 Keys recibidas:', Object.keys(body));
    ...
    if (!userLink) {
      console.error(`❌ Usuario Moodle ${quizData.userid} no está vinculado a Telegram`);
      return NextResponse.json({
        success: false,
    ...
    console.log(`✅ Usuario vinculado encontrado: ${userLink.moodleusername} -> @${userLink.telegramuserid}`);
    ...
    for (const question of questions) {
      ...
      // ✅ MANEJAR AMBOS CAMPOS: iscorrect (simulado) y correct (observer real)
      const iscorrect = Boolean(question.iscorrect !== undefined ? question.iscorrect : question.correct);
      
      // ✅ USAR SOLO CAMPOS QUE EXISTEN EN LA TABLA REAL
      ...
      console.log(`✅ Actividad creada: ${activity.id} - Correcta: ${activity.questionCorrect}`);
    }

    console.log(`🎉 PROCESAMIENTO COMPLETADO: ${processedActivities.length} actividades creadas`);
    ...
    console.error('❌ Error procesando webhook Moodle:', error);
    return NextResponse.json({
      success: false,
