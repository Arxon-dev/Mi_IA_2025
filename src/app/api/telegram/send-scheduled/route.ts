...
import { shuffleOptionsForTelegram } from '@/utils/questionUtils'; // <--- AÑADIR IMPORTACIÓN
// import { TelegramService } from '@/services/telegramService'; // Descomentar si tienes este servicio

// Placeholder para la función de envío real a Telegram
async function sendQuestionToTelegram(botToken: string, chatid: string, question: SchedulableQuestion): Promise<{success: boolean, messageId?: string, error?: string}> {
  ...
  if (question.sourcemodel === 'validQuestion') {
    // ✅ NUEVO: Usar datos pre-parseados de ValidQuestion
    try {
      ...
      if (!quizData) {
        ...
        return {
          success: false,
          error: `Datos JSON inválidos en ValidQuestion ID: ${question.id}`
        };
      }
      
      console.log(`[sendQuestionToTelegram] ✅ Usando datos PRE-PARSEADOS de ValidQuestion:`);
      console.log(`   📝 Pregunta: "${quizData.question.substring(0, 100)}..."`);
      console.log(`   🔘 Opciones: ${quizData.options.length}`);
      console.log(`   ✅ Respuesta correcta: ${quizData.correctanswerindex + 1}`);
    } catch (error) {
    ...
    // ⚠️ LEGACY: Para compatibilidad con question/sectionQuestion
    const initialQuizData = prepareQuizData(question.content);
    console.log(`[sendQuestionToTelegram] Explicación extraída por prepareQuizData (truncada a 200 chars):`, initialQuizData?.explanationText?.substring(0, 200) + '...');
    console.log(`[sendQuestionToTelegram] Explicación completa extraída (si existe):`, initialQuizData?.explanationText);

    if (!initialQuizData) {
      console.error(`[sendQuestionToTelegram] prepareQuizData falló para ID: ${question.id}.`);
  ...
  // Validación final de quizData
  if (!quizData) {
    console.error(`[sendQuestionToTelegram] quizData es null para pregunta ID: ${question.id}`);
    return {
      success: false,
      error: `Datos de quiz no válidos para pregunta ID: ${question.id}`
    };
  }

  // Validar que hay suficientes opciones
  if (!quizData.options || quizData.options.length < 2) {
    console.error(`[sendQuestionToTelegram] La pregunta ID ${question.id} no tiene suficientes opciones válidas (${quizData.options?.length || 0}).`);
    return {
      success: false,
  ...
  // Límite de caracteres para la pregunta en Telegram (documentado: 300)
  ...
  if (questionForTelegram.length > TELEGRAM_QUESTION_MAX_LENGTH) {
    console.warn(`[sendQuestionToTelegram] La pregunta para la pregunta ID ${question.id} excede el límite de Telegram (${TELEGRAM_QUESTION_MAX_LENGTH} caracteres). Truncando...`);
  ...
  const pollPayload: any = {
    ...
    type: 'quiz',
    correct_option_id: randomizedQuizData.correct_option_id, // Usar índice correcto de randomizedQuizData
    is_anonymous: false,
  };

  // Límite de caracteres para la explicación en Telegram (documentado: 200 para quizzes)
  const TELEGRAM_EXPLANATION_MAX_LENGTH = 200;

  if (randomizedQuizData.explanation) { // Usar explicación de randomizedQuizData
    ...
    console.log(`[sendQuestionToTelegram] Explicación extraída por prepareQuizData (completa):`, explanationForTelegram);
    
    if (explanationForTelegram.length > TELEGRAM_EXPLANATION_MAX_LENGTH) {
      console.warn(`[sendQuestionToTelegram] La explicación para la pregunta ID ${question.id} excede el límite de Telegram (${TELEGRAM_EXPLANATION_MAX_LENGTH} caracteres). Truncando...`);
      explanationForTelegram = explanationForTelegram.substring(0, TELEGRAM_EXPLANATION_MAX_LENGTH - 1) + '…'; // Truncate and add ellipsis
      console.log(`[sendQuestionToTelegram] Explicación truncada a ${TELEGRAM_EXPLANATION_MAX_LENGTH} chars:`, explanationForTelegram.substring(0, 200) + '...'); // Log truncated version for debugging
    }
    ...
    console.log(`[sendQuestionToTelegram] Explicación FINAL en payload:`, pollPayload.explanation ? pollPayload.explanation.substring(0, 200) + '... (log muestra 200/maxLength)' : 'undefined');
    
    pollPayload.explanation_parse_mode = 'HTML'; // Asumiendo que la explicación puede tener HTML simple
  }
...
export async function POST(request: Request) {
  // TODO: Añadir un mecanismo de seguridad (ej. token secreto) para asegurar que solo el cron job llama a este endpoint.
  ...
  try {
    ...
    if (!aiConfig) {
      console.warn('[Scheduler] No se encontró AIConfig. Abortando envío programado.');
      return NextResponse.json({ message: 'AIConfig no encontrada.' }, { status: 404 });
    }

    if (!aiConfig.telegramSchedulerEnabled) {
      console.log('[Scheduler] Los envíos programados de Telegram están deshabilitados. Abortando.');
      return NextResponse.json({ message: 'Envío programado de Telegram deshabilitado.' });
    }

    // Lógica de franja horaria mejorada (hora:minuto y cruce de medianoche)
    const now = new Date();
    ...
    if (!isWithinSchedule) {
      console.log(`[Scheduler] Fuera de la franja horaria permitida (${aiConfig.telegramSchedulerStartHour ?? 8}:${(aiConfig.telegramSchedulerStartMinute ?? 0).toString().padStart(2, '0')}-${aiConfig.telegramSchedulerEndHour ?? 22}:${(aiConfig.telegramSchedulerEndMinute ?? 0).toString().padStart(2, '0')}). No se envía nada.`);
    ...
    if (!botToken || !chatid) {
      console.error('[Scheduler] Error: TELEGRAM_BOT_TOKEN o TELEGRAM_CHAT_ID no están configurados.');
      return NextResponse.json({ error: 'Configuración de Telegram incompleta en el servidor.' }, { status: 500 });
    }

    const quantity = aiConfig.telegramSchedulerQuantity || 1;
    const minimumIntervalDays = 30; // Período mínimo antes de que una pregunta pueda repetirse (en días)
    console.log(`[Scheduler] Intentando enviar ${quantity} quizzes con intervalo mínimo de repetición de ${minimumIntervalDays} días.`);

    let successfulSends = 0;
    let skip = 0;
    const MAX_QUESTIONS_TO_PROCESS = quantity * 200; // Límite para evitar bucles infinitos (ej: procesar hasta 200 veces la cantidad)
    let totalQuestionsProcessed = 0;

    console.log(`[Scheduler] Buscando hasta encontrar ${quantity} quizzes válidos (límite de procesamiento: ${MAX_QUESTIONS_TO_PROCESS} preguntas totales).`);

    while (successfulSends < quantity && totalQuestionsProcessed < MAX_QUESTIONS_TO_PROCESS) {
      console.log(`[Scheduler] Intento de búsqueda: Lote con skip=${skip}.`);
      
      // Usar la función mejorada que incluye el período mínimo de reenvío
      ...
      if (!questionsBatch || questionsBatch.length === 0) {
        console.log(`[Scheduler] No se encontraron más preguntas en la base de datos a partir de skip=${skip}.`);
      ...
      for (const question of questionsBatch) {
        ...
        try {
          ...
          // 2. Actualizar la información en la base de datos con el resultado (éxito o error)
          ...
          if (sendResult.success) {
            successfulSends++;
            console.log(`[Scheduler] -> Éxito en el envío de quiz ID: ${question.id}. Total enviados: ${successfulSends}/${quantity}.`);
          } else {
    ...
    console.log(`[Scheduler] Proceso de envío programado finalizado. ${successfulSends}/${quantity} quizzes enviados exitosamente.`);

    if (successfulSends === 0 && totalQuestionsProcessed >= MAX_QUESTIONS_TO_PROCESS) {
      console.warn('[Scheduler] No se pudieron enviar quizzes. Se alcanzó el límite máximo de preguntas procesadas sin encontrar suficientes válidas.');
      return NextResponse.json({ message: 'No se pudieron enviar quizzes. Límite de procesamiento alcanzado.' }, { status: 500 });
    }

    return NextResponse.json({ message: `Envío programado completado. ${successfulSends} quizzes enviados.` });

  } catch (error) {
    console.error('[Scheduler] Error catastrófico en el endpoint send-scheduled:', error);
    return NextResponse.json({ error: 'Error interno del servidor durante el envío programado.' }, { status: 500 });
  }
} 